...
- **Gotcha de plataforma (mobile)**: o isolamento estrito de node_modules do pnpm quebra resolução de plugins do babel do ecossistema Expo/RN (ex.: `@babel/plugin-transform-react-jsx` não resolvia a partir de `babel-preset-expo`). Corrigido com `.npmrc` → `node-linker=hoisted` na raiz do monorepo (afeta `apps/api`/`packages/shared` também, mas ambos foram revalidados depois — testes e build seguem passando). Não reverter esse `.npmrc` sem testar o mobile de novo.
- **Próximo passo natural**: issue #18 (client HTTP gerado via `orval` + autenticação) — depende do setup da #17, que está pronto.
