...
- **Fase 2 iniciada**: backlog completo em `docs/backlog-fase2.md` (issues #10-#15).
  ...
  - **Achado crítico desta issue, corrigido**: `packages/shared` nunca tinha sido compilado de verdade (`build` era um `echo` placeholder) — `node dist/main.js` (like produção roda) não resolvia `@minhasaude/shared` em runtime, só funcionava sob `ts-jest`/`ts-node` dos testes. Passou despercebido nas issues #10-#13. Corrigido com `tsconfig.build.json` + `main`/`types` apontando pra `dist/`. **Pendência que só o usuário pode resolver**: confirmar se o build command do serviço no Render já builda `@minhasaude/shared` antes da API (senão o próximo deploy quebra) — não dá pra checar/mudar isso sem a `RENDER_API_KEY`. Ver detalhes em `docs/backlog-fase2.md` issue #14.
- **Próximo passo natural**: Issue #15 (diário alimentar — CRUD, cópia de dia, resumo vs. meta), a última da Fase 2, conforme `docs/backlog-fase2.md`. Antes de dar deploy dessa leva de commits, resolver a pendência do build command do Render acima.
