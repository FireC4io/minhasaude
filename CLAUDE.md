...
- **Fase 3 (app mobile) planejada, execução não iniciada**: backlog em `docs/backlog-fase3.md`, issues abertas de verdade no GitHub como **#17-#21** (setup, client+auth, onboarding, diário+resumo, peso+gráfico, nessa ordem). Decisões de stack travadas: `orval` para o client OpenAPI (gera hooks TanStack Query direto, ver ADR-0003 atualizada) e `NativeWind` para estilização (ADR-0008, nova). `apps/mobile` hoje ainda é só placeholder (`apps/mobile/package.json`) — nada foi scaffoldado ainda.
- **Próximo passo natural**: discutir design (cores, formas, símbolos) antes do scaffold, depois começar pela issue #17 (setup Expo + Expo Router + EAS + NativeWind) — ver `docs/backlog-fase3.md`.
