...
- **Fase 2 iniciada**: backlog completo em `docs/backlog-fase2.md` (issues #10-#15).
  ...
  - **Issue #13 concluída** — catálogo de alimentos: entidades `Food`/`FoodPortion`, `search_vector` gerado (`STORED`, sem `unaccent` porque não é `IMMUTABLE`), índices GIN (`search_vector` + trigram em `name`), extensões `pg_trgm`/`unaccent` habilitadas — tudo via SQL bruto hand-editado na migration porque TypeORM não modela operator class declarativamente. CSV oficial da TACO reaproveitado de um repo MIT no GitHub (`ThiagoCarreiraVallim/fatia`, achado via `gh search code`) e salvo em `src/database/seeds/data/taco.csv`; seed idempotente (`pnpm --filter @minhasaude/api seed:taco`) importou **582 de 597 itens** — 15 pulados por proteína/carb/kcal `NA` na fonte (nunca vira `0` fabricado). `POST/PATCH/DELETE /v1/foods` + `GET /v1/foods/:id`: `source`/`owner_user_id` sempre server-side, edição de alimento alheio ou do sistema (TACO/OFF) retorna 404 (não 403, evita leak de existência). 16 testes novos (12 unitários + 4 e2e).
  - **Issue #14 concluída** — `GET /v1/foods/search?q=&page=&limit=`: trigram + `unaccent` local (função `immutable_unaccent()` + índice GIN funcional, já que `unaccent()` puro não é `IMMUTABLE`), fallback pro Open Food Facts (`cgi/search.pl` — só a v1 tem full-text search, v2/v3 só filtra por tag) quando a busca local vem vazia, cache automático do resultado (`source=off`, índice único parcial evita duplicar em buscas concorrentes), rate limit 20/min. Verificado ao vivo contra a API real do OFF (não só mockado): busca "nutella" cacheou 4 produtos reais, segunda busca voltou só do cache local.
  - **Achado crítico desta issue, corrigido em produção (2026-09-18)**: `packages/shared` nunca tinha sido compilado de verdade (`build` era um `echo` placeholder) — `node dist/main.js` (como produção roda) não resolvia `@minhasaude/shared` em runtime, só funcionava sob `ts-jest`/`ts-node` dos testes. Passou despercebido nas issues #10-#13. Corrigido localmente com `tsconfig.build.json` + `main`/`types` apontando pra `dist/`, **e** o `buildCommand` do serviço no Render foi atualizado via API pra `pnpm exec turbo run build --filter=@minhasaude/api` (builda o `shared` antes, via `dependsOn` do `turbo.json`). Deploy manual disparado pra validar: ficou `live`, `/health` e `/docs` responderam 200 em produção de verdade. Ver detalhes em `docs/backlog-fase2.md` issue #14.
  - **Auto-deploy no push**: o dashboard do Render mostra `Auto-Deploy: On Commit` configurado — pode já estar funcionando (README dizia que não, desde a Fase 1, mas não foi reverificado desde então). Status real só confirmado no próximo push de commits de verdade (vai acontecer naturalmente ao publicar a issue #15) — se não disparar sozinho, still falta conectar o GitHub App via OAuth no dashboard (Settings do serviço ou Account Settings → GitHub).
- **Próximo passo natural**: Issue #15 (diário alimentar — CRUD, cópia de dia, resumo vs. meta), a última da Fase 2, conforme `docs/backlog-fase2.md`.
