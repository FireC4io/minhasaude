...
- **Fase 2 iniciada**: backlog completo em `docs/backlog-fase2.md` (issues #10-#15). **Issue #10 concluída** — catálogo de calculadoras de gasto energético (`packages/shared/src/calculators/`): Mifflin-St Jeor, Katch-McArdle, TDEE por `activity_level`, distribuição de macros por objetivo. 14 testes unitários com valores conferidos contra a literatura (fórmulas verificadas via busca web nesta sessão, não só memorizadas). Pacote `@minhasaude/shared` ganhou infra de verdade nesta sessão (jest+ts-jest, eslint, tsconfig) — antes era só placeholder.
- **Próximo passo natural**: Issue #11 (`body_measurements` — peso manual, pré-requisito de input para a Issue #12/metas) ou #13 (catálogo de alimentos/TACO) em paralelo, conforme `docs/backlog-fase2.md`.
